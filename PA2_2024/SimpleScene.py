...
# global variables
wld2cam=[] # list of 4x4 camera-to-world matrices.
cam2wld=[] # list of 4x4 world-to-camera matrices.
cow2wld=None # 4x4 matrix storing modeling transformation for cow.
cursorOnCowBoundingBox=False # cursor is on the cow Bounding Box.
pickInfo=None # used for picking.
floorTexID=0 # texture for floor.
cameras= [
...
def spline(t, p0, p1, p2, p3) :
    return (0.5*((2*p1)*np.power(t,0) + (-p0 + p2)*np.power(t,1) + (2*p0 - 5*p1 + 4*p2 - p3)*np.power(t,2) + (-p0 + 3*p1 - 3*p2 + p3)*np.power(t,3)))
...
def onMouseButton(window,button, state, mods):
    ...
    global pick, pick_pos, cow2wld, cowpos, animStartTime
    GLFW_DOWN=1;
    GLFW_UP=0;
